...
{
    this->_map = new cc::Map(ASSET_DIR "/maps/2.5d-large.tmx", this->_renderer);
    cc::SpriteTexture::loadTexture("player1", ASSET_DIR "/res/player.png", this->_renderer, 90, 90, 4, 4);
...
void cc::Game::handleEvents()
{
    ...
    while(SDL_PollEvent(&e) != 0) {
        if(e.type == SDL_QUIT)
            ...
            this->_heldKeys.insert(e.key.keysym.sym);
        else if(e.type == SDL_KEYUP)
            this->_heldKeys.erase(e.key.keysym.sym);
    }
}

void cc::Game::update()
{
    float deltaTime = this->_timer.getDeltaTime() / 1000.0f;
...
void cc::Game::updatePlayerInput()
{
    float dx = 0.0f, dy = 0.0f;
    if(_heldKeys.count(SDLK_w))
        dy -= 1.0f;
    if(_heldKeys.count(SDLK_s))
        dy += 1.0f;
    if(_heldKeys.count(SDLK_a))
        dx -= 1.0f;
    if(_heldKeys.count(SDLK_d))
        dx += 1.0f;
    _player->setDirection(cc::Point(dx, dy));
}

void cc::Game::updateCameraInput()
{
    const float cameraSpeed = 300.0f;
    float dt = _timer.getDeltaTime() / 1000.0f;
    float dx = 0.0f, dy = 0.0f;
    if(_heldKeys.count(SDLK_LEFT))
        dx += cameraSpeed * dt;
    if(_heldKeys.count(SDLK_RIGHT))
        dx -= cameraSpeed * dt;
    if(_heldKeys.count(SDLK_UP))
        dy += cameraSpeed * dt;
    if(_heldKeys.count(SDLK_DOWN))
        dy -= cameraSpeed * dt;
    if(dx != 0.0f || dy != 0.0f)
        _camera.move(cc::Point(dx, dy));
    if(_heldKeys.count(SDLK_LEFTBRACKET))
        this->scale(1 / 16.0f);
    if(_heldKeys.count(SDLK_RIGHTBRACKET))
        this->scale(-1 / 16.0f);
}
